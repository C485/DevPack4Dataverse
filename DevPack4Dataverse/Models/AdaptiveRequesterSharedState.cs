...
/// <summary>
/// Represents the shared state used by the <see cref="AdaptiveRequester"/> class to store response time and request count data.
/// </summary>
public class AdaptiveRequesterSharedState
{
    ...
    /// </summary>
    /// <returns>A tuple containing the EMA of response times, the EMA of request counts, and the calculated requests per second.</returns>
    public (TimeSpan EmaResponseTime, double EmaRequestCount, double requestsPerSecond) CalculateEmaWithDynamicWeight()
    {
        ...
        double requestsPer = CalculateRequestsPerSecond(
            ResponseTimeBuffer[BufferIndex],
